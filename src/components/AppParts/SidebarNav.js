...
class AppSidebarNav extends Component {
  ...
  // nav list
  navList(items, isRestricted=true) {
    return items.map((item, index) => this.navType(item, index, isRestricted));
  }

  // nav type
  navType(item, idx, isRestricted) {
    if (isRestricted && item.restricted){
      return;
    } else {
      return (
        item.title ? this.navTitle(item, idx)
          : item.divider ? this.navDivider(item, idx)
            : item.label ? this.navLabel(item, idx)
              : item.children ? this.navDropdown(item, idx, isRestricted)
  ...
  // nav dropdown
  navDropdown(item, key, isRestricted=true) {
    ...
    return (
      <li key={key} className={this.activeRoute(item.url, this.props)}>
        ...
        <ul className="nav-dropdown-items">
          {this.navList(item.children, isRestricted)}
        </ul>
      </li>);
  }
  ...
  render() {
    ...
    return (
      <PerfectScrollbar className={navClasses} {...attributes} option={{ suppressScrollX: !isRtl }} >
        <Nav>
          {children || this.navList(navConfig.items, this.isRestricted)}
        </Nav>
      </PerfectScrollbar>
    );
  }
